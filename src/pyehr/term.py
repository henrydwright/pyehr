...
class ListCodeSetAccess(ICodeSetAccess):
    ...
    def __init__(self, id: str, lang: str, code_list: list):
        """Initialise a code set access list
        
        :param code_list: list[CodePhrase]"""
        self._id = id
        self._language = lang
...
class DictTerminologyAccess(ITerminologyAccess):
    ...
    _name : str
    _language : str
    _groupid_code_dict : dict[str, dict] #[str, dict[str, CodePhrase]]
    _groupname_code_dict : dict[str, dict] #[str, dict[str, CodePhrase]]
    _all_code_dict: dict #dict[str, CodePhrase]

    def __init__(self, id: str, lang: str, code_dict: dict[tuple[str, str], list]):
        """Initialise a dict based terminology access interface.
        
        :param code_dict: `dict[tuple[str, str], list[CodePhrase]]`"""
        self._id = id
        self._language = lang
...
TERMINOLOGYID_OPENEHR_INTEGRITY_CHECK_ALGORITHMS = TerminologyID("openehr_integrity_check_algorithms")
...
def generate_default_code_sets():
    """Fills the default codelists at runtime to prevent circular dependency issues."""
    ...
    CODELIST_OPENEHR_INTEGRITY_CHECK_ALGORITHMS = [
        CodePhrase(TERMINOLOGYID_OPENEHR_INTEGRITY_CHECK_ALGORITHMS, "SHA-1"),
        CodePhrase(TERMINOLOGYID_OPENEHR_INTEGRITY_CHECK_ALGORITHMS, "SHA-224"),
        CodePhrase(TERMINOLOGYID_OPENEHR_INTEGRITY_CHECK_ALGORITHMS, "SHA-256"),
        CodePhrase(TERMINOLOGYID_OPENEHR_INTEGRITY_CHECK_ALGORITHMS, "SHA-384"),
        CodePhrase(TERMINOLOGYID_OPENEHR_INTEGRITY_CHECK_ALGORITHMS, "SHA-512"),
        CodePhrase(TERMINOLOGYID_OPENEHR_INTEGRITY_CHECK_ALGORITHMS, "SHA-512/224"),
        CodePhrase(TERMINOLOGYID_OPENEHR_INTEGRITY_CHECK_ALGORITHMS, "SHA-512/256")
    ]
    ...
    CODESET_OPENEHR_INTEGRITY_CHECK_ALGORITHMS = ListCodeSetAccess(TERMINOLOGYID_OPENEHR_INTEGRITY_CHECK_ALGORITHMS.name(), "en", CODELIST_OPENEHR_INTEGRITY_CHECK_ALGORITHMS)
    ...
    return (CODESET_OPENEHR_CHARACTER_SETS, CODESET_OPENEHR_COUNTRIES, CODESET_OPENEHR_LANGUAGES, CODESET_OPENEHR_MEDIA_TYPES, CODESET_OPENEHR_COMPRESSION_ALGORITHMS, CODESET_OPENEHR_INTEGRITY_CHECK_ALGORITHMS, CODESET_OPENEHR_NORMAL_STATUSES)

def generate_default_terminology():
    """Fills the default terminology at runtime to prevent circular dependency issues."""
    from pyehr.core.rm.data_types.text import CodePhrase
    ...
    return TERMINOLOGY_OPENEHR

class PyehrGlobalTerminologyService():

    @staticmethod
    def get_global_terminology_service() -> TerminologyService:
        """Get a TERMINOLOGY_SERVICE intended to be used wherever it is not explicitly
        specified at class instantiation."""
        CODESET_OPENEHR_CHARACTER_SETS, CODESET_OPENEHR_COUNTRIES, CODESET_OPENEHR_LANGUAGES, CODESET_OPENEHR_MEDIA_TYPES, CODESET_OPENEHR_COMPRESSION_ALGORITHMS, CODESET_OPENEHR_INTEGRITY_CHECK_ALGORITHMS, CODESET_OPENEHR_NORMAL_STATUSES = generate_default_code_sets()
        TERMINOLOGY_OPENEHR = generate_default_terminology()
        _global_terminology_service = PythonTerminologyService(
            code_sets=[CODESET_OPENEHR_LANGUAGES, 
                    CODESET_OPENEHR_COUNTRIES, 
                    CODESET_OPENEHR_CHARACTER_SETS, 
                    CODESET_OPENEHR_MEDIA_TYPES, 
                    CODESET_OPENEHR_INTEGRITY_CHECK_ALGORITHMS, 
                    CODESET_OPENEHR_COMPRESSION_ALGORITHMS, 
                    CODESET_OPENEHR_NORMAL_STATUSES], 
            terminologies=[TERMINOLOGY_OPENEHR])
        
        return _global_terminology_service